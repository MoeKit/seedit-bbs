define("moe/seedit.bbs/1.0.0/seedit.bbs-debug", [], function(require, exports, module) {
    var url = document.location.href;
    var nodeReg1 = /forum-(\d+)-\d+.html/;
    var nodeReg2 = /mod=forumdisplay&fid=(\d+)/;
    var topicReg1 = /thread-(\d+)-\d+-\d+.html/;
    var topicReg2 = /mod=viewthread&tid=(\d+)/;
    var getTid = function() {
        if (window.tid) {
            return window.tid;
        }
        var temp = url.match(topicReg1);
        if (temp && temp[1]) {
            return temp[1];
        }
        temp = url.match(topicReg2);
        if (temp && temp[1]) {
            return temp[1];
        }
        return undefined;
    };
    var getFid = function() {
        if (window.fid) {
            return window.fid;
        }
        var temp = url.match(nodeReg1);
        if (temp && temp[1]) {
            return temp[1];
        }
        temp = url.match(nodeReg2);
        if (temp && temp[1]) {
            return temp[1];
        }
        return undefined;
    };
    exports.page = {
        // 是否为wap站
        isWap: function() {},
        // 是否为版块页面
        isNode: function() {
            return nodeReg1.test(url) || nodeReg2.test(url);
        },
        // 是否为帖子页面
        isTopic: function() {
            return topicReg1.test(url) || topicReg2.test(url);
        },
        // 获得tid
        getTid: getTid,
        // 获得fid
        getFid: getFid
    };
    exports.user = {
        // 是否已经登录
        isLogin: function() {}
    };
});
